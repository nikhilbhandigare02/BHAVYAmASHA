...
class TrackEligibleCoupleBloc
    extends Bloc<TrackEligibleCoupleEvent, TrackEligibleCoupleState> {
  ...
  TrackEligibleCoupleBloc({
    ...
    on<FpAntraInjectionDateChanged>((event, emit) {
      emit(state.copyWith(
        antraInjectionDateChanged: event.date,
        status: state.isValid ? FormStatus.valid : FormStatus.initial,
        clearError: true,
      ));
    });
    ...
    on<BeneficiaryAbsentCHanged>((event, emit) {
      emit(state.copyWith(
        beneficiaryAbsentCHanged: event.value,
        status: state.isValid ? FormStatus.valid : FormStatus.initial,
        clearError: true,
      ));
    });
    ...
    on<RemovalReasonChanged>((event, emit) {
      emit(state.copyWith(
        removalReasonChanged: event.method,
        status: state.isValid ? FormStatus.valid : FormStatus.initial,
        clearError: true,
      ));
    });
    ...
    //
    //     final financialYear =
    //         formData['financial_year']?.toString() ?? state.financialYear;
    ...
    //     final fpAdopting =
    //         formData['fp_adopting'] as bool? ?? state.fpAdopting;
    ...
    on<SubmitTrackForm>((event, emit) async {
      ...
      try {
        ...
        final formName =
            FollowupFormDataTable.formDisplayNames[formType] ??
                'Track Eligible Couple';
        final formsRefKey =
        ...
        final formData = {
          ...
          'form_data': {
            ...
            'protection_status':
            state.fpAdopting == true ? 'Protected' : 'Unprotected',
            ...
            'antra_injection_date':
            state.antraInjectionDateChanged?.toIso8601String(),
        ...
        try {
          final formId =
          await LocalStorageDao.instance.insertFollowupFormData(formDataForDb);

          if (formId > 0) {
            try {
              await db.update(
                'beneficiaries',
                {'is_family_planning': state.fpAdopting == true ? 1 : 0},
                where: 'unique_key LIKE ? OR id = ?',
                whereArgs: [
                  '%${state.beneficiaryId}',
                  int.tryParse(state.beneficiaryId) ?? 0
                ],
              );
              print(
                  'Updated beneficiary is_family_planning to ${state.fpAdopting == true ? 1 : 0}');
            } catch (e) {
            ...
            try {
              final secureStorageKey =
                  'ec_tracking_${state.beneficiaryId}_${DateTime.now().millisecondsSinceEpoch}';
              await _secureStorage.write(
                key: secureStorageKey,
                value: formJson,
              );
              print('Form data stored securely with key: $secureStorageKey');
            } catch (e) {
  ...
  Future<void> _loadPreviousFormData() async {
    try {
      ...
      formDataKeys.sort((a, b) {
        final aTime = int.tryParse(a.key.split('_').last) ?? 0;
        final bTime = int.tryParse(b.key.split('_').last) ?? 0;
