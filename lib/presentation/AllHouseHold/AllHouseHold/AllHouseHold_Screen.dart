...
// Calculate eligible couples per household
      ...
      for (final row in rows) {
        try {
          ...
          if (row['is_deleted'] == 1 ||
              ...
              row['is_death'] == 1) {
            final name = (info['headName'] ?? info['memberName'] ?? info['name'] ?? 'Unknown').toString();
            debugPrint('AllHouseHold: Skipping beneficiary $name - deleted: ${row['is_deleted']}, migrated: ${row['is_migrated']}, death: ${row['is_death']}');
            continue;
          }
          ...
          if (dob != null && dob.toString().isNotEmpty) {
            ...
            if (birthDate != null) {
              ...
              if (totalMonths >= 65 * 12) {
                ...
                debugPrint('  - Added to elderly (65+) category - Age: ${ageDetails['ageText']}');
              }
            }
          }
        } catch (_) {}
      ...
      final mapped = familyHeads.map<Map<String, dynamic>>((r) {
        ...
        for (final b in membersForHousehold) {
          ...
          try {
            final isHeadA = rawInfo['isFamilyhead'] == true || (rawInfo['isFamilyhead']?.toString().toLowerCase() == 'true');
            final isFamilyheadWife = rawInfo['isFamilyheadWife'] == true || (rawInfo['isFamilyheadWife']?.toString().toLowerCase() == 'true');

            if(isHeadA){
              totalLivechild = rawInfo['totalLiveChildren']?? 0;
            }
            if(isHeadA||isFamilyheadWife){
              continue;
            }
            else {
              recordedChildren++;
        ...
        final remainingChildren = totalLivechild - recordedChildren;
